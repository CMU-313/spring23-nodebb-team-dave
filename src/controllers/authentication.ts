...
export function registerAbort(req, res) {
    if (req.uid) {
...
export function continueLogin(strategy, req, res, next) {
    passport.authenticate(strategy, async (err, userData, info) => {
        ...
        if (userData.passwordExpiry && userData.passwordExpiry < Date.now()) {
            ...
            await doLogin(req, userData.uid);
            let destination;
            if (req.session.returnTo) {
...
export async function doLogin(req, uid) {
    if (!uid) {
        return;
    }
    const loginAsync = util.promisify(req.login).bind(req);
    await loginAsync({ uid: uid }, { keepSessionInfo: req.res.locals !== false });
    await onSuccessfulLogin(req, uid);
};

export async function onSuccessfulLogin(req, uid) {
    /*
...
export async function localLogin(req, username, password, next) {
    if (!username) {
